...
export default function FarmerOrders() {
  ...
  const load = async () => {
    try {
      ...
      // ✅ Backend already implements: GET /api/orders/farmer/sales
      // (see your order.routes.js)
  ...
  const updateStatus = async (orderId, status) => {
    try {
      ...
      // ❗Your backend does NOT have this route yet:
      // PATCH /api/orders/:id/status
      // So this call will 404 until you add the backend route.
    ...
    <div style={{ padding: 16 }}>
      ...
      {loading ? (
        ...
        <div style={{ display: "grid", gap: 12, marginTop: 16 }}>
          {orders.map((o) => {
            // keep display robust: payment info may be in o.payment instead of flat fields
      ...
      <div style={{ marginTop: 18, fontSize: 12, color: "#666" }}>
        Your farmer orders endpoint is <code>/api/orders/farmer/sales</code> (✅ already exists).
        If you get <b>404</b> for <code>/api/orders/:id/status</code>, you still need to add that backend route.
      </div>
    </div>
  );
}
